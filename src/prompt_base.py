...
from openprompt.utils import signature,round_list
...
class Template(nn.Module):
    ...
    def _check_template_format(self, ):
        ...
        """
        return None
        # because, we use target template don't use mask
        mask_num = 0
    ...
    def wrap_one_example(self,
        ...
        if isinstance(example, InputExample):
            ...
            for placeholder_token in self.placeholder_mapping:
                if not (placeholder_token=="<question_type>" or placeholder_token=="<answer>" or placeholder_token=="<context>" or placeholder_token=="<question>"):
...
class TokenizerWrapper:
    ...
    def truncate(self, encoder_inputs,max_seq_length):
        total_tokens = sum([len(part) for part in encoder_inputs['input_ids']])
        num_specials = self.num_special_tokens_to_add
        num_tokens_to_truncate = total_tokens - max_seq_length + num_specials
